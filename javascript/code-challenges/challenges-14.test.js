...
const toTitleCase = (arr) => arr.map(str => {
  let titleCaseArr = [...str];
  titleCaseArr[0] = titleCaseArr[0].toUpperCase();
...
const sortBy = (property, arr) => {
  return (property === 'price') ? sortByPrice(arr) : sortByName(arr);
};
...
const detectTicTacToeWin = (board) => {
  const checking = (matching) => {
    let [x1, y1, x2, y2, x3, y3] = matching;
  ...
  matchSets.forEach(matchPossibility => {
    if (checking(matchPossibility)) winner = true;
  });
