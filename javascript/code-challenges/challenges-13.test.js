...
const longestString = (arr) => {
  let maxLength = '';
  arr.map(elem => {
    if (elem.length > maxLength.length) {
      maxLength = elem;
    }
  });
...
const findHappiness = (arr) => {
  let newArr = arr.filter(result => result.includes(':)'));
...
const standardizePhoneNumbers = (arr) => {
  let newArr = arr.map(result => result.substring(1, 4) + result.substring(6, 9) + result.substring(10, 14));
  return newArr;
};
